...
def detect_lead_grid(raw: bytes) -> Optional[List[Tuple[int, int, int, int]]]:
    ...
    try:
        ...
        vert_x = []
        for line in lines:
            # OpenCV sürümüne göre HoughLinesP çıktısı (N, 1, 4) veya
            # (N, 4) olabilir. Her iki durumda da line[0] kullanmak,
            # ikinci biçimde ilk koordinat olan numpy.int32 değerini
            # dört değişkene ayırmaya çalışır.
...
def _gemini_generate_with_timeout(iv: List[bytes], model: str, prompt: Optional[str],
                                  timeout: int) -> Dict[str, Any]:
    """Gemini cagrilarini gercekten timeout ile calistir."""
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
        ...
        try:
            ...
            raise TimeoutError(f"Gemini {model} {timeout} saniye icinde yanit vermedi")
...
def gemini_oku(iv: List[bytes], model: str = "gemini-flash-latest",
    ...
    for m in candidates:
        try:
            return _gemini_generate_with_timeout(iv, m, prompt, timeout)
        except Exception as e:
            last_err = e
            err_str = str(e)
            if "404" in err_str or "NOT_FOUND" in err_str or "is not found" in err_str:
                ...
                continue
            logging.warning("Gemini model %s hata: %s", m, e)
            raise
...
<div class="mode">
<label><input type="radio" name="mode" value="fast" checked> 🚀 Hizli</label>
<label><input type="radio" name="mode" value="auto"> ⚡ Otomatik</label>
...
function run(url,mkfd){ab.disabled=true;mb.disabled=true;rs.style.display='block';rc.innerHTML='<div class="loading"><div class="spinner"></div><p style="color:var(--mut)">AI\'lar mm olcuyor → Grid temizleniyor → Kural motoru → Hakem → Internet dogrulama...</p></div>';var opt=mkfd?{method:'POST',body:mkfd()}:{};fetch(url,opt).then(async function(r){var text='';try{text=await r.text();return JSON.parse(text);}catch(je){console.error('JSON parse hatasi:',text,je);throw new Error((text||'Sunucu bos yanit verdi. Terminali kontrol edin.').substring(0,500));}}).then(function(r){if(r.status==='success')show(r.prediction);else rc.innerHTML='<div class="glass card" style="border-left:4px solid var(--red)"><b>Hata:</b> '+r.message+'</div>';}).catch(function(e){rc.innerHTML='<div class="glass card" style="border-left:4px solid var(--red)"><b>Hata:</b> '+e.message+'</div>';}).finally(function(){ab.disabled=false;mb.disabled=false;});}
...
async def analyze(file: UploadFile = File(...), mode: str = "fast", grid: bool = True):
    try:
        c = await file.read()
        if not c:
            return JSONResponse(status_code=400, content={"status": "error", "message": "Dosya bos"})
        if mode not in ("auto", "fast", "deep"):
            mode = "fast"
        img_hash = _hash(c)
        # Global timeout: 60 saniye
        try:
            p = await asyncio.wait_for(asyncio.to_thread(ensemble_analyze, c, mode=mode, grid_clean=grid), timeout=60.0)
        except asyncio.TimeoutError:
            return JSONResponse(status_code=504, content={"status": "error", "message": "Analiz cok uzun surdu (60 sn). Hizli modu deneyin veya API baglantisini kontrol edin."})
